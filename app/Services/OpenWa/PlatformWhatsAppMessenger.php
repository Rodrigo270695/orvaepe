...
/**
 * Envía WhatsApp desde la sesión OpenWA de plataforma Orvae.
 */
final class PlatformWhatsAppMessenger
{
    ...
    private function resolveReadySessionId(): string
    {
        $session = $this->sync->ensure();
        if ($session === null) {
            throw new RuntimeException('OpenWA no está configurado para Orvae.');
        }
        ...
        if (! $session->isReady()) {
            throw new RuntimeException('Sesión OpenWA de Orvae no está conectada (status: '.$session->status.').');
        }

        $sessionId = trim((string) $session->openwa_session_id);
        if ($sessionId === '') {
            throw new RuntimeException('Sesión OpenWA de Orvae sin id.');
        }

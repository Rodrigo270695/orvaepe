...
/**
 * Provisión de empresa OmniDesk / SendSaaS tras compra de plan en Orvae.
 * Orvae solo cobra; features y límites viven en el hijo.
 */
class SendSaaSPlanProvisioner
{
    ...
    public function provision(Order $order, CatalogSku $sku, ?\DateTimeInterface $periodEnd = null): void
    {
        if (! SaasCatalogSku::isSendsaas($sku)) {
            $this->noteSnapshot($order, ['sendsaas_provision_skipped' => 'sku_no_es_sendsaas']);

            return;
        }

        if (! $this->isEnabled()) {
            $this->noteSnapshot($order, ['sendsaas_provision_skipped' => 'SENDSAAS_PROVISIONING_ENABLED=false']);
            Log::warning('sendsaas.provision_skipped_disabled', [
                'order_id' => $order->id,
        ...
        $url = trim((string) config('services.sendsaas.provision_url'));
        $secret = (string) config('services.sendsaas.hmac_secret');

        if ($url === '' || $secret === '') {
            $this->noteSnapshot($order, ['sendsaas_provision_skipped' => 'falta_url_o_hmac_secret']);
            Log::warning('sendsaas.provision_skipped_missing_config', [
                'order_id' => $order->id,
                'order_number' => $order->order_number,
            ]);
        ...
        if (! $user instanceof User) {
            $this->noteSnapshot($order, ['sendsaas_provision_skipped' => 'pedido_sin_usuario']);

            return;
        }

        $existingSubscription = SaasSubscriptionLookup::findSendsaasRenewable((string) $user->id, $sku)
        ...
        if ($planSlug === null) {
            $this->noteSnapshot($order, [
                'sendsaas_provision_skipped' => 'plan_slug_no_mapeado',
                'sku_code' => $sku->code,
            ]);
            Log::info('sendsaas.provision_skipped_unmapped_plan', [
                'order_id' => $order->id,
                'sku_code' => $sku->code,
            ]);
        ...
        $payload = [
            ...
            'admin_apellidos' => trim((string) ($user->lastname ?? '')) ?: 'Empresa',
        ...
        Log::info('sendsaas.provision_request', [
            'order_id' => $order->id,
            'order_number' => $order->order_number,
            'plan_slug' => $planSlug,
            'tenant_slug' => $tenantSlug,
        ...
        if (! $response->successful()) {
            $this->noteSnapshot($order, [
                'sendsaas_provision_error' => [
                    'http_status' => $response->status(),
                    'body' => Str::limit((string) $response->body(), 2000),
                    'at' => now()->toIso8601String(),
                ],
            ]);
            Log::warning('sendsaas.provision_failed', [
                'order_id' => $order->id,
        ...
        if ((! is_string($loginUrl) || $loginUrl === '') && is_string($tenantSlugResp) && $tenantSlugResp !== '') {
            $loginUrl = $this->buildLoginUrl($tenantSlugResp);
        }
        ...
        Log::info('sendsaas.provision_success', [
            'order_id' => $order->id,
            'login_url' => $loginUrl,
    ...
    public function renew(
        ...
        if (! SaasCatalogSku::isSendsaas($sku) || ! $this->isEnabled()) {
            return;
        }
        ...
        if ($url === '' || $secret === '') {
            $this->noteSnapshot($order, ['sendsaas_renew_skipped' => 'falta_url_o_hmac_secret']);
        ...
        $tenantSlug = SaasSubscriptionLookup::sendsaasTenantSlugFrom($existingSubscription);
        if ($tenantSlug === null) {
            $this->noteSnapshot($order, ['sendsaas_renew_skipped' => 'sin_tenant_slug']);
        ...
        Log::info('sendsaas.renew_request', [
            'order_id' => $order->id,
            'tenant_slug' => $tenantSlug,
            'plan_slug' => $planSlug,
        ]);
        ...
        if (! $response->successful()) {
            $this->noteSnapshot($order, [
                'sendsaas_renew_error' => [
                    'http_status' => $response->status(),
                    'body' => Str::limit((string) $response->body(), 2000),
                    'at' => now()->toIso8601String(),
                ],
            ]);
            Log::warning('sendsaas.renew_failed', [
                'order_id' => $order->id,
                'status' => $response->status(),
            ]);
        ...
        Log::info('sendsaas.renew_success', [
            'order_id' => $order->id,
            'tenant_slug' => $tenantSlugResp,
        ]);
    }
    ...
    private function resolveTenantName(User $user): string
    {
        ...
        if ($fullName !== '') {
            return 'Empresa '.$fullName;
        }

        return 'Empresa '.(string) $user->email;
    }
    ...
    private function resolveTenantSlug(string $tenantName, string $email): string
    {
        ...
        return $slug !== '' ? Str::limit($slug, 60, '') : 'empresa-'.Str::lower(Str::random(8));
    }
    ...
    {
        $explicit = trim((string) config('services.sendsaas.renew_url', ''));
        if ($explicit !== '') {
            return $explicit;
        }

        $provision = trim((string) config('services.sendsaas.provision_url', ''));
        if ($provision === '') {
            return '';
        }
    ...
    private function buildLoginUrl(string $tenantSlug): string
    {
        ...
        return sprintf('%s://%s.%s/login', $scheme, $tenantSlug, $domain);
    }
    ...
    private function persistRenewalAccess(
        ...
        $metadata['sendsaas_login_url'] = $url;
        $metadata['sendsaas_tenant_slug'] = $slug;
        ...
        unset($snapshot['sendsaas_renew_error'], $snapshot['sendsaas_renew_skipped']);
        $snapshot['sendsaas_login_url'] = $url;
        $snapshot['sendsaas_tenant_slug'] = $slug;
        $snapshot['sendsaas_renewed_at'] = now()->toIso8601String();
        $order->forceFill(['billing_snapshot' => $snapshot])->save();
    }

    private function isEnabled(): bool
    {
        return (bool) config('services.sendsaas.enabled', false);
    }
    ...
    private function findRenewableByOrderTenantSlug(Order $order, CatalogSku $sku): ?Subscription
    {
        ...
        return SaasSubscriptionLookup::findSendsaasByTenantSlug($tenantSlug, $sku);
    }

    private function renewTenantSlugFromOrder(Order $order): ?string
    {
        $snapshot = is_array($order->billing_snapshot) ? $order->billing_snapshot : [];
        $slug = $snapshot['sendsaas_renew_tenant_slug'] ?? null;

        if (is_string($slug) && trim($slug) !== '') {
            return trim($slug);
        }

        $sessionSlug = session('sendsaas_renew_tenant_slug');
    ...
    private function persistAccess(
        ...
        if ($subscription instanceof Subscription) {
            $metadata = is_array($subscription->metadata) ? $subscription->metadata : [];
            $metadata['sendsaas_login_url'] = $url;
            $metadata['sendsaas_bootstrap_url'] = $bootstrap;
            $metadata['sendsaas_tenant_slug'] = $slug;
            $metadata['sendsaas_initial_password_sent'] = true;
            $subscription->forceFill(['metadata' => $metadata])->save();
        }

        $snapshot = is_array($order->billing_snapshot) ? $order->billing_snapshot : [];
        unset($snapshot['sendsaas_provision_error'], $snapshot['sendsaas_provision_skipped']);
        $snapshot['sendsaas_login_url'] = $url;
        $snapshot['sendsaas_bootstrap_url'] = $bootstrap;
        $snapshot['sendsaas_tenant_slug'] = $slug;
        $snapshot['sendsaas_login_email'] = $order->user?->email;
        $snapshot['sendsaas_temporary_password'] = $temporaryPassword;
        $order->forceFill(['billing_snapshot' => $snapshot])->save();
    }
}

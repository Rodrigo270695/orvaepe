...
export default function ScrollToTopButton({ className = '' }: { className?: string }) {
    ...
    return (
        <button
            ...
            onClick={scrollToTop}
            className={[
                // Por encima del botón flotante de WhatsApp (bottom-6 + h-14 + separación)
                'fixed bottom-[calc(1.5rem+3.5rem+0.75rem)] right-4 z-50 flex size-12 cursor-pointer items-center justify-center rounded-full',
                ...
                'sm:right-6 md:size-14 md:right-8',
                className,
            ].join(' ')}
